...
void BLE_PROTOCOL_CmdFrameProcess(BLE_PROTOCOL_CB* pCB)
{
	...
	switch (cmd)
	{
		...
		case BLE_CMD_FIRMUPDATE_DOWN:
			...
			//LCD_PrintString(50, 160, "Please BLE Update...", LCD_COLOR_BLACK, LCD_OPERATOR_MODE_DRAW);
			LCD_PrintString(10, 50, "BLE Updating...", LCD_COLOR_WHITE, LCD_OPERATOR_MODE_DRAW);
			
			// 动态显示
			TIMER_AddTask(TIMER_ID_BLE_UPDATE_STATE,
							1000,
							LCD_DotDynamicShow,
							UINT32_NULL,
							TIMER_LOOP_FOREVER,
		...
		case BLE_CMD_DATAIN_DOWN:
			...
			temp <<= 8;
			temp += pCmdFrame->buff[IAP_CTRL_UART_CMD_DATA4_INDEX];
			writeRes = IAP_BleWriteAppBin(IAP_FLASH_APP2_ADDR + temp * 128, &pCmdFrame->buff[IAP_CTRL_UART_CMD_DATA5_INDEX], 128);
			if(writeRes)
			{
		...
		case BLE_CMD_DATAIN_FINISH_DOWN:
			...
			LCD_PrintString(10, 50, "BLE Updating...    ", LCD_COLOR_BLACK, LCD_OPERATOR_MODE_DRAW);
			if(recCrc8 == tempCrc8)
			{
				...
				// 数据包校验成功之后，跳转应用程序运行
				LCD_PrintString(10, 50, "BLE Update Success!", LCD_COLOR_WHITE, LCD_OPERATOR_MODE_DRAW);
			...
			else
			{
				...
				LCD_PrintString(10, 50, "BLE Update failed!", LCD_COLOR_WHITE, LCD_OPERATOR_MODE_DRAW);
