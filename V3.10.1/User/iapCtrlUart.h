...
#define IAP_CTRL_UART_TYPE_DEF					UART3							// 选择串口(串口中断和时钟都需要修改)
#define IAP_CTRL_UART_IRQn_DEF					UART3_IRQn 					// 选择串口(串口中断和时钟都需要修改)
...
typedef enum{
	...
	IAP_CTRL_UART_CMD_DOWN_PARAM_SET = 0x10,					// 系统参数写入
	IAP_CTRL_UART_CMD_UP_PARAM_SET_RESULT = 0x11,				// 系统参数写入结果
	...
	IAP_CTRL_UART_CMD_CHECK_VERSION = 0x90,
	
	IAP_CTRL_UART_CMD_FLAG_ARRAY = 0x91,
	IAP_CTRL_UART_CMD_TEST_LCD = 0x92,

	IAP_CTRL_UART_CMD_FCT_DATA_WRITE = 0x93,
	IAP_CTRL_UART_CMD_FCT_TEST_FLAG_WRITE = 0x94,
