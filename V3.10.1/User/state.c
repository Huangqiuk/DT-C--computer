...
void STATE_EnterState(uint32 state)
{
	...
	switch (state)
	{
		...
		case STATE_NULL:
			...
			updateFlag = SPI_FLASH_ReadByte(SPI_FLASH_HOST_INFO_OFFSET_ADDR);
		...
		case STATE_ENTRY:
			...
			// 升级数据包大小
			upAppDataSize = SPI_FLASH_ReadWord(SPI_FLASH_HOST_INFO_OFFSET_ADDR+4);
	...
	switch (stateCB.state)
	{
		...
		case STATE_ENTRY:
				...
				if (i > ((upAppDataSize / SPI_FLASH_APP_READ_UPDATA_SIZE) + 1))
				{
					...
					// 清除蓝牙升级标志和屏蔽蓝牙模块初始化标志
					SPI_FLASH_WriteByte(SPI_FLASH_HOST_INFO_OFFSET_ADDR, 0x00);
				}
				
				SPI_FLASH_ReadArray(buff, SPI_FLASH_HOST_APP_OFFSET_ADDR + i*SPI_FLASH_APP_READ_UPDATA_SIZE, SPI_FLASH_APP_READ_UPDATA_SIZE);
