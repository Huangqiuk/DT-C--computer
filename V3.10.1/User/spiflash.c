...
void SPIx_FLASH_Init(void)
{
	...
	// APB2 - SPI0 - 72M
	// APB1 - SPI1/SPI2 - 36M
	spi_init_struct.prescale             = SPI_PSC_4;							// 72/2=36M
...
void DMAy_SPIx_Configuration(void)
{
    ...
    dma_rx_init_struct.periph_width = DMA_PERIPHERAL_WIDTH_8BIT;	// 外设的传输数据宽度16位
    dma_rx_init_struct.memory_width = DMA_MEMORY_WIDTH_8BIT;		// 存储器的传输数据宽度16位
    ...
    dma_tx_init_struct.memory_width = DMA_MEMORY_WIDTH_8BIT;		// 存储器的传输数据宽度16位
    dma_tx_init_struct.periph_width = DMA_PERIPHERAL_WIDTH_8BIT;	// 外设的传输数据宽度16位
