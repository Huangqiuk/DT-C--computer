...
const uint32 PARAM_MCU_VERSION_ADDRESS[] = {SPI_FLASH_SN_ADDEESS, SPI_FLASH_CUSTOMER_SN_ADDEESS, SPI_FLASH_HW_VERSION_ADDEESS, \
											SPI_FLASH_BOOT_VERSION_ADDEESS, SPI_FLASH_APP_VERSION_ADDEESS, SPI_FLASH_BLE_UPDATA_FLAG_ADDEESS, \
											SPI_FLASH_BLE_UPDATA_CRC_ADDEESS, SPI_FLASH_NEW_QR_CODE_ADDEESS, SPI_FLASH_UI_VERSION_ADDEESS};
...
void PARAM_SetDefaultRunningData(void)
{
	...
	// 读取标志区数据
	SPI_FLASH_ReadArray(paramCB.runtime.flagArr, SPI_FLASH_TEST_FLAG_ADDEESS, 64);
	...
	// 验证BOOT版本
	SPI_FLASH_ReadArray(&length, SPI_FLASH_BOOT_VERSION_ADDEESS, 1);
	if (IAP_VERSION_LENGTH != length)
	{
		length = IAP_VERSION_LENGTH;
		SPI_FLASH_WriteByte(SPI_FLASH_BOOT_VERSION_ADDEESS, length);
	}
	
	// 读取BOOT版本字符
	SPI_FLASH_ReadArray(buf, SPI_FLASH_BOOT_VERSION_ADDEESS + 1, IAP_VERSION_LENGTH);
	
	// 如果读取的版本号不符，则强制更新为当前BOOT版本
	if (!PARAM_CmpareN((const uint8*)buf, (const uint8*)IAP_VERSION, IAP_VERSION_LENGTH)) 
	{
		SPI_FLASH_WriteWithErase((uint8*)IAP_VERSION, SPI_FLASH_BOOT_VERSION_ADDEESS + 1, IAP_VERSION_LENGTH);
	}

	// 读取APP版本字符
	pBuff = &(paramCB.runtime.appVersion[0]);
	pBuff[0] = SPI_FLASH_ReadByte(SPI_FLASH_APP_VERSION_ADDEESS);
	if (31 < pBuff[0])
	{
		pBuff[0] = 3;
		pBuff[1] = 'E';
	...
	{
		SPI_FLASH_ReadArray(&(pBuff[1]), SPI_FLASH_APP_VERSION_ADDEESS + 1, pBuff[0]);
	}

	// 读取二维码版本字符
	pBuff = &(paramCB.runtime.qrCodeStr[0]);
	pBuff[0] = SPI_FLASH_ReadByte(SPI_FLASH_NEW_QR_CODE_ADDEESS);
	if (63 < pBuff[0])
	{
		pBuff[0] = 3;
		pBuff[1] = 'E';
	...
	{
		SPI_FLASH_ReadArray(&(pBuff[1]), SPI_FLASH_NEW_QR_CODE_ADDEESS + 1, pBuff[0]);
	}
...
void Param_Process(void)
{
	...
	if ((paramCB.runtime.qrRegEnable) && (bleUartCB.rcvBleMacOK) && (gsmUartCB.rcvIccidOK || gsmUartCB.rcvIccidTimeOut))
	{
		...
		*/
		...
		// 拷贝字符串
		strcpy((char*)hmiTypeStr, (const char*)"BC28Uxx");
...
//=====================================================================================================================
	// 出厂参数
	if (paramCB.factoryNvmWriteRequest)
	{
		paramCB.factoryNvmWriteRequest = FALSE;

		// 写入参数
		SPI_FLASH_WriteWithErase(paramCB.factoryNvm.array, SPI_FLASH_FACTORY_PARAM_ADDEESS, PARAM_FACTORY_NVM_DATA_SIZE);
