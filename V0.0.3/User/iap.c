...
***************************************IAP封装接口函数使用步骤****************************************************
	...
	注：为了防止程序跑飞，在进行程序跳转时要对程序判断：if(((*(vu32*)(IAP_FLASH_APP1_ADDR+4))&0xFF000000)==IAP_GD32_FLASH_BASE),
	    如果不满足条件，则不能跳转至应用程序。
	Others: 		//
	Function List:	// 
	1. ....
...
BOOL IAP_WriteAppBin(uint32 appAddr, uint8 *appBuf, uint32 appSize)
{
	...
	uint16 iapBuf[1024];			// 此16位数据需要和串口通讯协议一帧最长字节有关126*2=252个字节
	BOOL writeStatus;
...
uint8 IAP_FlashReadForCrc8(uint32 datalen)
{
	...
	while (len--)
	{
		dataTemp = *(volatile uint32*)(addrIndex + IAP_FLASH_APP1_ADDR);
		addrIndex += 4;
		// 小端模式
