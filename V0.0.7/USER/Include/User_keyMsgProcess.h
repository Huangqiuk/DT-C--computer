#ifndef 	__USER_KEY_MSG_PROCESS_MODULE_H__
#define 	__USER_KEY_MSG_PROCESS_MODULE_H__
...
#define	KEY_SCAN_TIME			20		// 按键扫描周期，单位：ms，必须为虚拟定时器的分辨率的N倍(N>=1)
...
typedef enum
{
	KEY_MASK_M = (1UL<<BUTTON_M),
	KEY_MASK_DEC = (1UL<<BUTTON_DEC),
	KEY_MASK_ADD = (1UL<<BUTTON_ADD),
	
	KEY_MASK_M_DEC = (1UL<<BUTTON_M_DEC),
	KEY_MASK_M_ADD = (1UL<<BUTTON_M_ADD),
	KEY_MASK_DEC_ADD = (1UL<<BUTTON_DEC_ADD),
...
{
	uint8_t	end;
	uint16_t	stateCount[KEY_INFO_SIZE];				// 每个元素最高位为按键状态，0为按下，1为释放，其余位为记录次数
...
typedef struct
{
	volatile uint8_t	scanRequest;				// 按键扫描请求标志
	
	uint8_t	keyValue[KEY_STATE_BYTE_COUNT];			// 按键值，定时更新，每个bit代表一个按键状态，
	...
	bool addInfoKeyUpRercoveryRequest;				// + i键弹起恢复请求
	bool addKeyInvalidRequest;						// +键无效请求
	bool GroupKeyInvalidRequest;					// 组合键无效请求
}KEYMSG_CB;
...
// 按键消息响应注册接口，可以设置长按门限
void KEYMSG_RegisterMsgServiceWithHoldTime(KEY_NAME_E keyName, KEY_MSG_E keyMsg, void(*service)(uint32_t param), uint32_t param, uint16_t holdTime);
...
// 设置+ i键弹起恢复请求
void KEYMSG_SetAddInfoKeyUpRecoveryRequest(bool state);

// 获取+ i键弹起恢复请求
bool KEYMSG_GetAddInfoKeyUpRecoveryRequest(void);

// 设置组合键恢复请求
void KEYMSG_SetGroupKeyRecoveryRequest(bool state);

// 获取组合键恢复请求
bool KEYMSG_GetGroupKeyUpRecoveryRequest(void);
