...
*               V0.1.0 - 2017.5.2  by Sun Quanjin
*                        增加 CLICK 和 DBCLICK 两个消息
...
// 每个按键的长按门限
const uint16_t keyHoldThreshHold[KEY_NAME_MAX] = 
{
	// 常规触摸按键长按门限值
	1500,   	// 
	1500,   	// 
...
// 每个按键的重复周期
const uint16_t keyRepeatTime[KEY_NAME_MAX] = 
{
	// 常规触摸按键重复周期
	200,		// KEY_HELP
	200,		// KEY_BLE
...
void KEYMSG_ServiceDoNothing(uint32_t param);
...
void KEYMSG_Init(void)
{
	uint8_t i,j;
 	//创建定时器
	//esp_err_t err = esp_timer_create(&KEY_TIMER , &KEY_TIMER_Handle);//创建定时器
	//printf("Key Time Create:%s", err == ESP_OK ? "OK!\r\n" : "failed!\r\n");
	...
	keyMsgCB.GroupKeyInvalidRequest =  false;
	...
	//err = esp_timer_start_periodic(KEY_TIMER_Handle , KEY_SCAN_TIME*1000);//打开定时器
	//printf("Key Time Start:%s", err == ESP_OK ? "OK!\r\n" : "failed!\r\n");

	//按键状态清空，因为默认是高电平，就把状态赋值为高
 for(uint8_t i=0; i<KEY_NAME_MAX; i++){
		keyCB.buff[i]  = keyCB.lastValue[i] = 0xFF;
	}
	//启动定时器
...
void KEYMSG_ScanProcess(void)
{
	...
	uint16_t currentKeyBit = 0;	// 当前按键状态，无论哪个按键，都需要把按键值移动到最高位，以便与状态队列中的数值进行比较
	
	uint16_t currentCount = 0;	// 当前按键状态计数值
	uint16_t currentState = 0;	// 当前按键状态
	uint16_t* pStateCount = NULL;	// 当前操作中的状态计数值
	...
	//keyMsgCB.KeyUpdate = KEY_CALLBACK_Scan;

	if(NULL == keyMsgCB.KeyUpdate)
	{
		return ;
	}
	(*keyMsgCB.KeyUpdate)(keyMsgCB.keyValue);//读取键值
	...
	for(i=0; i<KEY_NAME_MAX; i++)
	{
		...
		//NRF_LOG_INFO("KEYPROCESS %d - %x-%x-%x",i ,currentState, currentCount,currentKeyBit);
		...
		{
			//ESP_LOGI("KEY CHANGE", "%d", keyMsgCB.keyValue[0]);
...
void KEYMSG_MsgProcess(void)
{
	...
	// 本段用于DOWN / HOLD / REPEAT / UP四个消息的判断与抛出
	for(i=0; i<KEY_NAME_MAX; i++)
	{
		// 获取当前按键信息结构体指针
		pKeyInfo = &keyMsgCB.keyMsgInfo[i];
...
uint8_t KEYMSG_ThrowMsg(KEY_NAME_E keyName, KEY_MSG_E keyMsg)
{
	NRF_LOG_INFO("KEYMSG_THrowMsg:%d - %d", keyName, keyMsg);
