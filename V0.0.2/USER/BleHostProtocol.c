...
{
//  uint8 bleStr[] = {"TTM:SCAN=1,10\r\n"};
//  uint8 bleStr[] = {"TTM:CONNECT=01:F0:64:6B:26:44,20,14\r\n"};
    uint8 bleStr[100] = {0};
    sprintf((char *)bleStr, (char *)"TTM:CONNECT=%s,20,14\r\n", uumacBuff);

//  // 读写handle固定
//  //sprintf((char *)bleStr, "TTM:CONNECT=%s\r\n", PARAM_GetHelmetMacBuff());
//  sprintf((char *)bleStr, "TTM:CONNECT=%s,20,14\r\n", "01:F0:64:6B:26:44");
...
void BLE_HOST_UART_TTM_AnalysisCmd(uint8 buff[], uint8 length)
{
    ...
    {
        connectFlag = 1;
        TIMER_KillTask(TIMER_ID_BLE_CONNECT_DUT);
        TIMER_KillTask(TIMER_ID_BLE_CONNECT_DUT_TIME_OUT);

        // 断开连接
        TIMER_AddTask(TIMER_ID_BLE_TEST_DISCONNECT,
                      BLE_OUTFIT_CONNECT_TIME,
                      BLE_RESET,
                      TRUE,
                      BLE_OUTFIT_PAIR_COUNT,
...
//        // 停止断开连接
//        TIMER_KillTask(TIMER_ID_BLE_TEST_DISCONNECT);
//        
//        UART_PROTOCOL_TxAddData(UART_PROTOCOL_CMD_HEAD);
//        UART_PROTOCOL_TxAddData(UART_PROTOCOL_CMD_DEVICE_ADDR);
//        UART_PROTOCOL_TxAddData(UART_ECO_CMD_BLUETOOTH_MAC_ADDRESS_READ);
//        UART_PROTOCOL_TxAddData(7);
//        UART_PROTOCOL_TxAddData(1);
//        UART_PROTOCOL_TxAddData((HexToChar(uumacBuff[0]) << 4) | (HexToChar(uumacBuff[1]) & 0x0F)); //D0
//        UART_PROTOCOL_TxAddData((HexToChar(uumacBuff[3]) << 4) | (HexToChar(uumacBuff[4]) & 0x0F)); //D0
//        UART_PROTOCOL_TxAddData((HexToChar(uumacBuff[6]) << 4) | (HexToChar(uumacBuff[7]) & 0x0F)); //D0
//        UART_PROTOCOL_TxAddData((HexToChar(uumacBuff[9]) << 4) | (HexToChar(uumacBuff[10]) & 0x0F)); //D0
//        UART_PROTOCOL_TxAddData((HexToChar(uumacBuff[12]) << 4) | (HexToChar(uumacBuff[13]) & 0x0F)); //D0
//        UART_PROTOCOL_TxAddData((HexToChar(uumacBuff[15]) << 4) | (HexToChar(uumacBuff[16]) & 0x0F)); //D0
//        UART_PROTOCOL_TxAddFrame();
//        
//    }
    if ((connectFlag) && (-1 != STRING_Find(buff, (uint8 *)"OK")))
    {
        connectFlag = 0;
...
void BLE_DISCONNECT(uint32 param)
{
    // 断开连接，方便下一次连接
    uint8 bleStr[100] = {0};
    sprintf((char *)bleStr, (char *)"TTM:DISCONNECT=%s\r\n", uumacBuff);
