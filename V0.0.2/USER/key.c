...
void KEY_CALLBACK_Scan(uint8* p8bitKeyValueArray)
{
	...
	keyRead = GPIO_ReadInputDataBit(GPIOA, GPIO_PIN_5); // 电源键
	keyRead <<= 1;
	keyRead |= GPIO_ReadInputDataBit(GPIOB, GPIO_PIN_4);//-
	keyRead <<= 1;	
	keyRead |= GPIO_ReadInputDataBit(GPIOB, GPIO_PIN_3);//+
	
	// ■■这里增加按键滤波措施■■
	// 【第一步】将读取到的按键放入缓冲区
	for(i=0; i<KEY_NAME_MAX-2; i++)		// 组合按键不需要在这里滤波，因此这里循环上限-1
	{
	...
	// 判断组合键是否开启
	if (!KEYMSG_GetGroupKeyUpRecoveryRequest())
	{
