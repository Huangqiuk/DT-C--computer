...
BOOL CAN_Tx_FillAndSend(CAN_CB *pCB)
{
    ...
    if (index < length)
    {
        ...
        {
            case DUT_TYPE_CM:
                if (dut_info.frameType == DUT_FRAME_TYPE_EXT)
                {
                    // 扩展帧
            ...
            default:
                ...
                if(DUT_TYPE_MEIDI == dut_info.ID)
                {
                    canTxMsgCB.tx_sfid = 0x00000753;                    
                }
                else
                {
                    canTxMsgCB.tx_sfid = standardID;                
...
void CAN_CmdFrameProcess(CAN_CB *pCB)
{
    ...
    for (sourceBuffSelect = 0; sourceBuffSelect < CAN_DEVICE_SOURCE_ID_MAX; sourceBuffSelect++)
    {
        ...
        switch (dut_info.ID)
        {
            ...
            case DUT_TYPE_CM: // 通用仪表升级
                switch (cmd)
                {
                    case IAP_CTRL_CAN_CMD_EMPTY: // 0x00 空命令
                        break;

                    case IAP_CTRL_CAN_CMD_UP_PROJECT_APPLY:
                        STATE_SwitchStep(STEP_CM_CAN_UPGRADE_WAIT_FOR_ACK);
                        if(dut_info.bootEaring)
                        {
                            STATE_SwitchStep(STEP_CM_CAN_UPGRADE_WAIT_FOR_ACK);                        
                        }
                        ...
                        else if (!dut_info.dutPowerOnAllow) // 只接收一次eco请求
                        {
                            STATE_SwitchStep(STEP_CM_CAN_UPGRADE_RECEIVE_DUT_ECO_REQUEST);
                            dut_info.dutPowerOnAllow = TRUE; // 不再接收eco请求
                        }
                        else if ((DUT_NAME_BC147 == dut_info.nam) && (dut_info.configUpFlag) && (!dut_info.dutPowerOnAllow))
                        {
                            STATE_SwitchStep(STEP_CAN_SET_CONFIG_ENTRY);
                        }
                        else if (writeUiVerFlag) // 只接收一次eco请求
                        {
                            STATE_SwitchStep(STEP_CM_CAN_WRITE_UI_VER);
                            writeUiVerFlag = FALSE;
                        }                        
                        break;

                    case IAP_CTRL_CAN_CMD_UP_PROJECT_READY: // 0x03：DUT工程模式准备就绪
                        TIMER_KillTask(TIMER_ID_DELAY_ENTER_UP_MODE);
                        if (dut_info.reconnectionFlag)
                        {
                            if (dut_info.reconnectionRepeatOne)
                            {
                                if (dut_info.appUpFlag)
                                {
                                    STATE_SwitchStep(STEP_CM_CAN_UPGRADE_SEND_APP_WRITE);
                                }
                                else if (dut_info.uiUpFlag)
                                {
                                    STATE_SwitchStep(STEP_CM_CAN_UPGRADE_SEND_UI_WRITE);
                                }
                            }
                            break;
                        }
                        ...
                        if (dut_info.appUpFlag)
                        {
                            STATE_SwitchStep(STEP_CM_CAN_UPGRADE_SEND_APP_EAR);
                        }
                        else if (dut_info.uiUpFlag)
                        {
                            STATE_SwitchStep(STEP_CM_CAN_UPGRADE_SEND_UI_EAR);
                        }
                    ...
                    case IAP_CTRL_CAN_CMD_UP_UI_DATA_ERASE_ACK: // 0x13,DUT_UI擦写成功
                        // 提取DUT_UI擦除结果
                        temp = pCmdFrame->buff[CAN_CMD_DATA1_INDEX];
                        // 擦除失败
                        if (0 == temp)
                        {
                            // 擦除失败再擦除一次
                        ...
                        // 擦除成功
                        STATE_SwitchStep(STEP_CM_CAN_UPGRADE_UI_EAR_RESUALT);
                        break;

                    case IAP_CTRL_CAN_CMD_UP_UI_DATA_WRITE_RESULT: // 0x15  //DUT_UI写入结果
                        temp = pCmdFrame->buff[CAN_CMD_DATA1_INDEX];

                        if (0 == temp)
                        {
                            dut_info.currentUiSize--; // 应答失败重发上一包数据
                        }
			           if (dut_info.currentUiSize < (dut_info.uiSize + 1))
                        ...
                        {
                            STATE_SwitchStep(STEP_CM_CAN_UPGRADE_UI_UP_SUCCESS);
                        }
                        break;

                    case IAP_CTRL_CAN_CMD_DOWN_IAP_ERASE_FLASH: // 0x24：DUT_APP擦写
                        break;

                    case IAP_CTRL_CAN_CMD_UP_IAP_ERASE_FLASH_RESULT: // 0x25：DUT_APP擦写结果查询
                        temp = pCmdFrame->buff[CAN_CMD_DATA1_INDEX];
                        // 擦除失败
                        if (0 == temp)
                        {
                        ...
                        STATE_SwitchStep(STEP_CM_CAN_UPGRADE_APP_EAR_RESUALT);
                        break;

                    case IAP_CTRL_CAN_CMD_UP_IAP_WRITE_FLASH_RESULT: // 0x27：DUT_APP写入结果
                        temp = pCmdFrame->buff[CAN_CMD_DATA1_INDEX];
                        ...
                        {
                            STATE_SwitchStep(STEP_CM_CAN_UPGRADE_APP_UP_SUCCESS);
                        }
                        break;

                    case IAP_CTRL_CAN_CMD_UP_UPDATA_FINISH_RESULT: // 0x2B：DUT写入结果确定
            ...
            case DUT_TYPE_HUAXIN:
                ...
                // 收到仪表eco请求
                case IAP_CTRL_CAN_CMD_DOWN_PROJECT_APPLY_ACK: // 0x02
                    if (dut_info.appUpFlag || dut_info.uiUpFlag)
                    {
                        if (!dut_info.dutPowerOnAllow) // 只接收一次eco请求
                        {
                            STATE_SwitchStep(STEP_HUAXIN_CAN_UPGRADE_RECEIVE_DUT_ECO_REQUEST);
                            dut_info.dutPowerOnAllow = TRUE; // 不再接收eco请求
                        }
                    }
                    else if (dut_info.configUpFlag)
                ...
                case IAP_CTRL_CAN_CMD_UP_PROJECT_READY: // 0x03：DUT工程模式准备就绪
                    ...
                    if (dut_info.appUpFlag)
                    {
                        STATE_SwitchStep(STEP_HUAXIN_CAN_UPGRADE_SEND_APP_EAR);
                    }
                    else if (dut_info.uiUpFlag)
                    {
                ...
                case IAP_CTRL_CAN_HUAXIN_CMD_APP_EAR: // 0x05：DUT_APP擦写结果查询
                    temp = pCmdFrame->buff[CAN_CMD_DATA1_INDEX];
                    // 擦除失败
                    if (0 == temp)
                    {
                    ...
                    STATE_SwitchStep(STEP_HUAXIN_CAN_UPGRADE_SEND_APP_WRITE);
                    break;

                case IAP_CTRL_CAN_HUAXIN_CMD_WRITE_APP: // 0x06：DUT_APP写入结果
                    temp = pCmdFrame->buff[CAN_CMD_DATA1_INDEX];
                    ...
                    {
                        STATE_SwitchStep(STEP_HUAXIN_CAN_UPGRADE_APP_UP_SUCCESS);
                    }
                    break;

                case IAP_CTRL_CAN_HUAXIN_CMD_WRITE_APP_COMPLETE: // 0x07：DUT写入结果确定
                    STATE_SwitchStep(STEP_HUAXIN_CAN_UPGRADE_ITEM_FINISH);
                    break;
                ...
                case IAP_CTRL_CAN_CMD_UP_UI_DATA_WRITE_RESULT: // 0x15：DUT_UI写入结果
                    ...
                    {
                        STATE_SwitchStep(STEP_HUAXIN_CAN_UPGRADE_UI_UP_SUCCESS);
                    }
                    break;

                case IAP_CTRL_CAN_CMD_UP_UPDATA_FINISH_RESULT:
                    STATE_SwitchStep(STEP_HUAXIN_CAN_UPGRADE_ITEM_FINISH);
                    break;
            ...
            case DUT_TYPE_SEG: // 段码屏
                // 执行命令帧
                switch (cmd)
                {
                // 收到仪表发送的ECO请求
                case IAP_CTRL_CAN_CMD_DOWN_PROJECT_APPLY_ACK: // 02
                ...
                // 仪表回应准备就绪
                case IAP_CTRL_CAN_CMD_UP_PROJECT_READY: // 03
                    if (dut_info.appUpFlag)
                    {
                        STATE_SwitchStep(STEP_SEG_CAN_UPGRADE_SEND_APP_EAR); // 擦除app
                    }
                    break;

                case IAP_CTRL_CAN_CMD_SEG_RUN_APP_ACK: // 04
                ...
                case IAP_CTRL_CAN_HUAXIN_CMD_APP_EAR: // 05
                    STATE_SwitchStep(STEP_SEG_CAN_UPGRADE_APP_EAR_RESUALT);
                    // 提取DUT_UI擦除结果
                    temp = pCmdFrame->buff[CAN_CMD_DATA1_INDEX];
                    ...
                    break;
                case IAP_CTRL_CAN_HUAXIN_CMD_WRITE_APP: // 06,app写入
                    temp = pCmdFrame->buff[CAN_CMD_DATA1_INDEX];
                    if (0 == temp) // 如果仪表写入失败
                    {
                        dut_info.currentAppSize--;
                ...
                case IAP_CTRL_CAN_HUAXIN_CMD_WRITE_APP_COMPLETE: // 07，升级项结束，开始升级下一项
            ...
            case DUT_TYPE_KAIYANG:
                ...
                {
                // 收到仪表发送的ECO请求
                case IAP_CTRL_CAN_CMD_UP_PROJECT_APPLY: // 01

                    if (!dut_info.dutPowerOnAllow)
                    {
                        if (dut_info.uiUpFlag || dut_info.appUpFlag)
                        {
                            STATE_SwitchStep(STEP_KAIYANG_CAN_UPGRADE_UP_ALLOW);
                        }
                        else if (dut_info.configUpFlag)
                        {
                            STATE_SwitchStep(STEP_CAN_SET_CONFIG_ENTRY);
                ...
                case IAP_CTRL_CAN_CMD_DOWN_PROJECT_APPLY_ACK: // 02
                    if (dut_info.appUpFlag)
                    {
                        ...
                        TIMER_AddTask(TIMER_ID_RECONNECTION,
                                      1000,
                                      STATE_SwitchStep,
                                      STEP_KAIYANG_CAN_UPGRADE_SEND_APP_AGAIN,
                                      TIMER_LOOP_FOREVER,
                    ...
                    else if (dut_info.uiUpFlag)
                    {
                        ...
                        TIMER_AddTask(TIMER_ID_RECONNECTION,
                                      1000,
                                      STATE_SwitchStep,
                                      STEP_KAIYANG_CAN_UPGRADE_SEND_UI_AGAIN,
                                      TIMER_LOOP_FOREVER,
                                      ACTION_MODE_ADD_TO_QUEUE);
                    }
                    break;
                ...
                case IAP_CTRL_CAN_CMD_UP_PROJECT_READY: // 03
                    ...
                    if (dut_info.appUpFlag)
                    {
                        ...
                        {
                            if (pCmdFrame->buff[CAN_CMD_DATA1_INDEX])
                            {
                                dut_info.currentAppSize++;
                            }
                            STATE_SwitchStep(STEP_KAIYANG_CAN_UPGRADE_SEND_APP_WRITE);
                        ...
                        {
                            STATE_SwitchStep(STEP_KAIYANG_CAN_UPGRADE_APP_UP_SUCCESS);
                        }
                    }
                    else if (dut_info.uiUpFlag)
                    {
                ...
                case IAP_CTRL_CAN_CMD_SEG_RUN_APP_ACK: // 04
                    ...
                    TIMER_KillTask(TIMER_ID_DUT_TIMEOUT);
                    STATE_SwitchStep(STEP_KAIYANG_CAN_UPGRADE_ITEM_FINISH);
                    break;
...
BOOL CAN_DRIVE_AddTxArray(uint32 id, uint8 *pArray, uint8 length)
{
    ...
    // BC147和280产品软件会过滤id信息
    if((DUT_NAME_BC147 == dut_info.nam) || (DUT_NAME_BC280 == dut_info.nam)) 
    {
       sysCanCB.tx.cmdQueue[end].deviceID = 0x1801FFF4;
    }
    else
    {
...
void USBD_LP_CAN0_RX0_IRQHandler(void)
{
    ...
    while (can_receive_message_length_get(CAN0, CAN_FIFO0))
    {
        ...
        case DUT_TYPE_GB:
            ...
            CAN_TEST_PROTOCOL_MacProcess(rxMessage1.rx_efid, &rxMessage1.rx_data[0], rxMessage1.rx_dlen);
            CAN_MD_PROTOCOL_MacProcess(rxMessage1.rx_efid, &rxMessage1.rx_data[0], rxMessage1.rx_dlen);
            break;
        ...
        default:
            ...
            CAN_TEST_PROTOCOL_MacProcess(rxMessage1.rx_efid, &rxMessage1.rx_data[0], rxMessage1.rx_dlen);
            CAN_MD_PROTOCOL_MacProcess(rxMessage1.rx_sfid, &rxMessage1.rx_data[0], rxMessage1.rx_dlen);
            break;
        }
    }
}
...
//        dut_info.reconnectionRepeatContent[6] = ((addr) & 0x0000FF00) >> 8;                   // 数据长度
//        dut_info.reconnectionRepeatContent[7] = ((addr) & 0x000000FF);                        // 数据长度
        SPI_FLASH_ReadArray(&dut_info.reconnectionRepeatContent[4], flashAddr + (addr), 132); // 工具读取128字节
        for (i = 0; i < 135; i++)                                                             // 添加数据
        {
            IAP_CTRL_CAN_TxAddData(dut_info.reconnectionRepeatContent[i + 1]);
        }    
