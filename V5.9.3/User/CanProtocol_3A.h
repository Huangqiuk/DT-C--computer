...
#define CAN_PROTOCOL1_RX_QUEUE_SIZE 3 // 接收命令队列尺寸
#define CAN_PROTOCOL1_TX_QUEUE_SIZE 3 // 发送命令队列尺寸

#define CAN_PROTOCOL1_CMD_HEAD 0x3A		   // 命令头
#define CAN_PROTOCOL1_CMD_DEVICE_ADDR 0x1A // 设备地址
#define CAN_PROTOCOL1_IOT_DEVICE_ADDR						0x2A	// IOT设备地址
...
typedef enum
{
    ...
    CAN_PROTOCOL1_CMD_READ_CONTROL_PARAM_REPORT = 0xC3,     // 控制器参数上报
 	...
 	CAN_PROTOCOL1_CMD_VERSION_TYPE_WRITE = 0xA8,			// 写入版本信息
	CAN_PROTOCOL1_CMD_VERSION_TYPE_READ = 0xA9,				// 读取版本信息   
	CAN_PROTOCOL1_CMD_DCD_FLAG_WRITE = 0xA7,				// 写入DCD标志（5）
	CAN_PROTOCOL1_CMD_DCD_FLAG_READ = 0xA6,				    // 读取DCD标志（5）
    
    // iot升级
	CAN_PROTOCOL1_CMD_UPDATE_REQUEST = 0xF1,		// 升级请求命令
	CAN_PROTOCOL1_CMD_UPDATE_START = 0xF2,			// 升级固件命令
	CAN_PROTOCOL1_CMD_WRITE_DATA = 0xF3,			// 数据包写入命令
	CAN_PROTOCOL1_CMD_WRITE_COMPLETE = 0xF4,		// 数据包写入完成命令    
