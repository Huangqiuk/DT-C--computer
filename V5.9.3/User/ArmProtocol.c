...
void dutProgressReport(uint32 temp)
{
    ...
    {
        dut_info.dutProgress[1] = (dut_info.currentBootSize * 100) / dut_info.bootSize;
    }
    else if (dut_info.appUpFlag)
    {
    ...
    if (dut_info.bootUpFaile) // boot升级失败
    {
        dut_info.bootUpFaile = FALSE;
        dut_info.dutProgress[1] = 102;
    ...
    if (dut_info.bootUpSuccesss)
    {
        dut_info.dutProgress[1] = 100;
        dut_info.currentBootSize = 0;
        dut_info.bootUpSuccesss = FALSE;
    }
...
void setUpItem(uint8 upItem, uint8 ctrlState)
{
    ...
    if (1 == ctrlState)
    {
        if (0x01 == (upItem & 0x01)) // 升级boot
        {
            dut_info.bootUpFlag = TRUE;
            dut_info.currentBootSize = 0;
        }
...
//  dut_info.dutProgress[1] = 101; // boot
    ...
    if (!dut_info.bootUpFlag)
    {
        dut_info.dutProgress[1] = 101;
    }
    else
    {
        dut_info.dutProgress[1] = 0;
    }
...
void enterState()
{
    if (dut_info.bootUpFlag || dut_info.appUpFlag || dut_info.uiUpFlag || dut_info.configUpFlag)
    {
        ...
        case 0: // 串口
            ...
            case DUT_TYPE_IOT:
                // 进入IOT升级
                STATE_EnterState(STATE_IOT_UART_UPGRADE);
                break;    
        ...
        case 1: // can
            ...
            case DUT_TYPE_MEIDI:
                // 进入MEIDI升级
                STATE_EnterState(STATE_MEIDI_CAN_UPGRADE);
                break;

            case DUT_TYPE_IOT:
                // 进入IOT升级
                STATE_EnterState(STATE_IOT_CAN_UPGRADE);
                break;    
...
void ARM_PROTOCOL_CmdFrameProcess(ARM_PROTOCOL_CB *pCB)
{
    ...
    case UART_DUT_CMD_UP_UI: // UI数据写入（命令字=0x31）
        ...
        {
            // 读取UI的flash区，然后写入DUT  4 + 128
            packetSize = pCmdFrame->buff[ARM_PROTOCOL_CMD_LENGTH_INDEX];
            addr1 = pCmdFrame->buff[ARM_PROTOCOL_CMD_DATA1_INDEX];
            addr2 = pCmdFrame->buff[ARM_PROTOCOL_CMD_DATA2_INDEX];
            addr3 = pCmdFrame->buff[ARM_PROTOCOL_CMD_DATA3_INDEX];
            addr4 = pCmdFrame->buff[ARM_PROTOCOL_CMD_DATA4_INDEX];
            ...
            if(packetSize != 132)
            {
                for(i = (pCmdFrame->buff[ARM_PROTOCOL_CMD_LENGTH_INDEX] + 2) ; i < 135 ;i++)
                {
                  pCmdFrame->buff[i + 1] = 0xFF;
                }
            }
            
            // 写入4 + 128 减1 从基地址开始写
            writeRes = SPI_FLASH_WriteWithCheck(&pCmdFrame->buff[ARM_PROTOCOL_CMD_DATA1_INDEX], SPI_FLASH_DUT_UI_ADDEESS + ((dut_info.uiSize - 1) * 132), 132);        
             
//            if((writeAddr == 0x002F9100) || (writeAddr == 0x00000000))
//            {            
//                // 读取验证
//                   SPI_FLASH_ReadArray(uiBuff, SPI_FLASH_DUT_UI_ADDEESS + ((dut_info.uiSize - 1) * 132), 132);
//            }
        }
        else
        {
    ...
    case UART_DUT_CMD_UP_APP:
        packetSize = pCmdFrame->buff[ARM_PROTOCOL_CMD_LENGTH_INDEX] - 4;
        dut_info.iotLastPackDateLen = packetSize;
        addr1 = pCmdFrame->buff[ARM_PROTOCOL_CMD_DATA1_INDEX];
        addr2 = pCmdFrame->buff[ARM_PROTOCOL_CMD_DATA2_INDEX];
        addr3 = pCmdFrame->buff[ARM_PROTOCOL_CMD_DATA3_INDEX];
        addr4 = pCmdFrame->buff[ARM_PROTOCOL_CMD_DATA4_INDEX];
    ...
    case UART_DUT_CMD_UP_END:
        ...
        {
            // BC550iot升级用
            dut_info.iotAppUpDataLen = ((dut_info.appSize - 1) * 128) + dut_info.iotLastPackDateLen;
            dut_info.iotCrc8 = SPI_FlashReadForCrc8(((dut_info.appSize - 1) * 128) + dut_info.iotLastPackDateLen);
            SPI_FLASH_EraseSector(SPI_FLASH_IOT_APP_UP_DATA_LEN);
            SPI_FLASH_WriteWord(SPI_FLASH_IOT_APP_UP_DATA_LEN, dut_info.iotAppUpDataLen);
            SPI_FLASH_EraseSector(SPI_FLASH_IOT_CRC8);
            SPI_FLASH_WriteWord(SPI_FLASH_IOT_CRC8, dut_info.iotCrc8);  
        }
    ...
    // UI版本号写入
    case UART_DUT_CMD_UP_UI_VER:
        SPI_FLASH_EraseSector(SPI_FLASH_UI_VERSION_ADDEESS);
        writeRes = SPI_FLASH_WriteWithCheck(&pCmdFrame->buff[ARM_PROTOCOL_CMD_LENGTH_INDEX], SPI_FLASH_UI_VERSION_ADDEESS, pCmdFrame->buff[ARM_PROTOCOL_CMD_LENGTH_INDEX] + 1);

        // 返回信息给安卓机
        ARM_PROTOCOL_SendCmdParamAck(UART_DUT_CMD_UP_UI_VER, writeRes);
        break;
