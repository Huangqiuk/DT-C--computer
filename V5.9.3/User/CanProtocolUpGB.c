...
void CAN_UP_DATA(uint32 param)
{
    ...
    if (dut_info.appUpFlag)
    {
        ...
        // 向发送buff添加地址编号
        canProtocolCB.sendBuff[0] = ((dut_info.currentAppSize >> 24) & 0xFF);
        canProtocolCB.sendBuff[1] = ((dut_info.currentAppSize >> 16) & 0xFF);
        canProtocolCB.sendBuff[2] = ((dut_info.currentAppSize >> 8) & 0xFF);
        canProtocolCB.sendBuff[3] = (dut_info.currentAppSize & 0xFF);
        // 向发送buff添加数据
        SPI_FLASH_ReadArray(&canProtocolCB.sendBuff[4], (SPI_FLASH_DUT_APP_ADDEESS + (dut_info.currentAppSize - 1) * 128), 128);
        // STATE_PGN_SEND_1：工具请求仪表接收数据
        CAN_PGN_RequestSendData(STATE_PGN_SEND_1, 0xEF, 0x28, 0xF9);
    }
