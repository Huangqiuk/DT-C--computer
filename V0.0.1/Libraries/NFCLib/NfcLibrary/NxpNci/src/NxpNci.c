...
bool NxpNci_PresenceCheck2(NxpNci_RfIntf_t RfIntf)
{
    bool status;
    uint8_t i;
    uint8_t Answer[MAX_NCI_FRAME_SIZE];
    ...
    case PROT_T5T:
        {
            ...
            if ((status == NXPNCI_SUCCESS) && (Answer[0] == 0x00) && (Answer[1] == 0x00) && (Answer[AnswerSize-1] == 0x00))
            {
				res = NXPNCI_ERROR;
            }
            else
...
bool NxpNci_ConfigureSettings(void)
{
    ...
    uint8_t currentTS[32] =  __TIME__" "__DATE__;
    ...
    if(!memcmp(&Answer[8], currentTS, sizeof(currentTS)) && (gRfSettingsRestored_flag == false))
    {
