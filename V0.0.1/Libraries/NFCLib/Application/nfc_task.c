...
uint8_t CheckCardPeriod;
uint8_t restartSuccessStatus;
uint8_t onlineStatus;

void virPrintf(char *Data,  ...)
{
	
}
#define PRINTF virPrintf//printf
#define PRINT_BUF(x,y,z)  {PRINTF("\n");}//{int loop; PRINTF(x); for(loop=0;loop<z;loop++) PRINTF("%.2x ", y[loop]); PRINTF("\n");}
...
void PCD_MIFARE_scenario (void)
{
    ...
    #define KEY_MFC         0xD3, 0xF7, 0xD3, 0xF7, 0xD3, 0xF7
...
void task_nfc_reader(NxpNci_RfIntf_t RfIntf)
{
    ...
    while(1){
		...
		{
//			case PROT_T1T:
//				cardIdLenght = RfIntf.Info.NFC_APP.NfcIdLen;
//				
//				cardId = RfIntf.Info.NFC_APP.NfcId;
//				break;
			...
			case PROT_T2T:
				...
				UART_PROTOCOL_SendCardId(RfIntf.Protocol, cardIdLenght, cardId);
				break;

			case PROT_T3T:
				cardIdLenght = 8;

				cardId = RfIntf.Info.NFC_FPP.SensRes;
			...
			case PROT_ISODEP:
				...
				switch (RfIntf.ModeTech)
				{
					...
					case TECH_PASSIVE_NFCB:
						cardIdLenght = 4;

						cardId = RfIntf.Info.NFC_BPP.SensRes;
...
void NFC_CheckCardOnline(uint32 param)
{
	CheckCardPeriod = 1;
}
...
void NFC_Init(void)
{
	...
	// 开启一个定时器，主要目的是检测到卡后间隔查询卡是否还在，防止耗大部分资源在查询卡状态上
    TIMER_AddTask(TIMER_ID_NFC_REMOVE,
					200,
					NFC_CheckCardOnline,
					bTRUE,
					TIMER_LOOP_FOREVER,
	...
	restartSuccessStatus = NFC_SUCCESS;
	CheckCardPeriod = 0;
	onlineStatus = 0;
}

//NxpNci_RfIntf_t saveRfInterface;
NxpNci_RfIntf_t RfInterface;
...
void NFC_Process(void)
{
	...
	// 卡在线时需要定时发波检测卡是否移除
	if ((onlineStatus || (NFC_ERROR == restartSuccessStatus)) && (CheckCardPeriod))
	{
		CheckCardPeriod = 0;

		// 最高优先级，没有启动成功首先考虑启动的事情
		if (NFC_ERROR == restartSuccessStatus)
		{
			NxpNci_StopDiscovery();
			if (NFC_ERROR == NxpNci_StartDiscovery(DiscoveryTechnologies,sizeof(DiscoveryTechnologies)))
		    {
				restartSuccessStatus = NFC_ERROR;
		    }
		    else
		    {
				restartSuccessStatus = NFC_SUCCESS;

				DDL_ZERO_STRUCT(RfInterface);
		    }
		...
		// 如果卡还没有移除不能重新启动
		if (NFC_ERROR == NxpNci_PresenceCheck2(RfInterface))
		{
			return;
		}
	    ...
	    NxpNci_StopDiscovery();
	    //NxpNci_StartDiscovery(DiscoveryTechnologies,sizeof(DiscoveryTechnologies));
	    //while (NFC_ERROR == NxpNci_StartDiscovery(DiscoveryTechnologies,sizeof(DiscoveryTechnologies)))
	    //{
		//	Sleep(1);
	    //}
	    if (NFC_ERROR == NxpNci_StartDiscovery(DiscoveryTechnologies,sizeof(DiscoveryTechnologies)))
	...
	// 是否有消息可读，判断的标准是IRQ是否为高
	if (NxpNci_WaitForDiscoveryNotification(&RfInterface) != NFC_SUCCESS)
	{
		return;
	}
	...
	if ((RfInterface.ModeTech & MODE_MASK) == MODE_POLL)
	{
		...
		onlineStatus = 1;
		CheckCardPeriod = 0;
		TIMER_ResetTimer(TIMER_ID_NFC_REMOVE);

		// 显示具体的充电电量界面
		paramCB.runtime.nfcFlag = 1;
	}
	else
#endif // ifdef RW_SUPPORT
	{
