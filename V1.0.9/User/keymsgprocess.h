...
{
	// 按键消息定制，最多支持四种消息
#if 1==KEY_MSG_SWITCH_DOWN      //// 消息响应开关，按键按下消息
...
// 按键信息节点，按键按下时最高位0（bit7），按键释放时最高位为1，低7位为按键状态计数值，使用时需要注意最大记录次数
typedef struct
{
	uint8	end;
...
}KEYMSG_INFO_CB;   //KEYMSG_INFO_CB结构体可以在按键消息处理模块中用来管理单个按键的状态、记录次数以及处理函数等相关信息，方便按键消息的处理和响应。
...
typedef struct
{
	...
	//当调用结构体中的KeyUpdate函数指针时，会将名为p8bitKeyValueArray的指针传递给该函数
...
// 按键消息响应注册接口，可以设置长按门限
void KEYMSG_RegisterMsgServiceWithHoldTime(KEY_NAME_E keyName, KEY_MSG_E keyMsg, void(*service)(uint32 param), uint32 param, uint16 holdTime);
