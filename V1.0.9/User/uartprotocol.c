...
//	UART_PROTOCOL_TxStateProcess();
	...
	// 3A配置协议
	// UART接收FIFO缓冲区处理
	UART_PROTOCOL4_RxFIFOProcess(&uartProtocolCB4);

	// UART接收命令缓冲区处理
	UART_PROTOCOL4_CmdFrameProcess(&uartProtocolCB4); // km5s协议，在app中升级config

	// UART协议层发送处理过程
	UART_PROTOCOL4_TxStateProcess();
}
...
void UART_PROTOCOL_TxAddFrame_3A(void)
{
	...
	UART_PROTOCOL4_TX_CMD_FRAME *pCmdFrame = &uartProtocolCB4.tx.cmdQueue[uartProtocolCB.tx.end];
	uint16 length = pCmdFrame->length;
...
//	// 添加命令头
//	UART_PROTOCOL4_TxAddData(UART_PROTOCOL4_CMD_HEAD);

//	// 添加设备地址
//	UART_PROTOCOL4_TxAddData(UART_PROTOCOL_CMD_DEVICE_ADDR);

//	// 添加命令字
//	UART_PROTOCOL4_TxAddData(UART_PROTOCOL_CMD_WRITE_CONTROL_PARAM); // 0xc0

//	// 添加数据长度
//	UART_PROTOCOL4_TxAddData(54);
        ...
        case DUT_TYPE_SEG:
            switch (cmd)
            {
                ...
                case UART_CMD_UP_APP_UP_OVER: // 07，app升级完毕
                    STATE_SwitchStep(STEP_CM_UART_SEG_UPGRADE_APP_UP_SUCCESS_RESET);
                    break;
