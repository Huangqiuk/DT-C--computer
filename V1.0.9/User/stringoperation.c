...
// 16进制高四位转char
unsigned char HexToCharHigh(unsigned char hexValue)
{
    unsigned char highNibble = (hexValue >> 4) & 0x0F; // 获取高四位的值
    if (highNibble >= 0x0 && highNibble <= 0x9)
    {
        return highNibble + 0x30; // 如果高四位是 '0' 到 '9'，则返回对应的字符值
    }
    else if (highNibble >= 0xA && highNibble <= 0xF)
    {
        return highNibble + 0x37; // 如果高四位是大写字母 A 到 F，则返回对应的字符值
    }
    else if (highNibble >= 0xa && highNibble <= 0xf)
    {
        return highNibble + 0x57; // 如果低四位是大写字母 a 到 f，则返回对应的字符值
    }
    else
    {
        return 0xff; // 如果高四位不在上述范围内，则返回 0xff，表示转换失败
    }
}

// 16进制低四位转char
unsigned char HexToCharLow(unsigned char hexValue)
{
    unsigned char lowNibble = hexValue & 0x0F; // 获取低四位的值
    if (lowNibble >= 0x0 && lowNibble <= 0x9)
    {
        return lowNibble + 0x30; // 如果低四位是 '0' 到 '9'，则返回对应的字符值
    }
    else if (lowNibble >= 0xA && lowNibble <= 0xF)
    {
        return lowNibble + 0x37; // 如果低四位是大写字母 A 到 F，则返回对应的字符值
    }
    else if (lowNibble >= 0xa && lowNibble <= 0xf)
    {
        return lowNibble + 0x57; // 如果低四位是大写字母 a 到 f，则返回对应的字符值
    }
    else
    {
        return 0xff; // 如果低四位不在上述范围内，则返回 0xff，表示转换失败
    }
}
