...
void STS_UART_HwInit(uint32 baud)
{
	...
	usart_deinit(STS_UART_TYPE_DEF); // 复位串口

	// 串口配置
	usart_baudrate_set(STS_UART_TYPE_DEF, baud);						  // 波特率
	usart_word_length_set(STS_UART_TYPE_DEF, USART_WL_8BIT);			  // 8位数据位
	usart_stop_bit_set(STS_UART_TYPE_DEF, USART_STB_1BIT);				  // 一个停止位
	usart_parity_config(STS_UART_TYPE_DEF, USART_PM_NONE);				  // 无奇偶校验
	usart_hardware_flow_rts_config(STS_UART_TYPE_DEF, USART_RTS_DISABLE); // 无硬件数据流控制
	usart_hardware_flow_cts_config(STS_UART_TYPE_DEF, USART_CTS_DISABLE);
	usart_transmit_config(STS_UART_TYPE_DEF, USART_TRANSMIT_ENABLE); // 使能发射
	usart_receive_config(STS_UART_TYPE_DEF, USART_RECEIVE_ENABLE);	 // 使能接收

	// 中断配置
	nvic_irq_enable(STS_UART_IRQn_DEF, 2, 1);

	usart_interrupt_enable(STS_UART_TYPE_DEF, USART_INT_RBNE); // 开启中断

	usart_enable(STS_UART_TYPE_DEF); // 使能串口
}
...
// 中断模式发送
	...
	if (usart_interrupt_flag_get(STS_UART_TYPE_DEF, USART_INT_FLAG_ERR_ORERR) != RESET) //----------------------- 接收溢出中断
	{
		usart_flag_get(STS_UART_TYPE_DEF, USART_FLAG_ORERR); 							//----------------------- 清除接收溢出中断标志位
		usart_data_receive(STS_UART_TYPE_DEF);				 							//----------------------- 清空寄存器
	}
}
