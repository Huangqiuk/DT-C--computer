...
void SPIx_FLASH_Init(void)
{
     ...
     * PB5 --> SPI1 MOSI Write  Mode: GPIO_Mode_AF_PP
     * PB4 --> SPI1 MISO Read   Mode: GPIO_Mode_IPU
     * PE1 --> SPI1 CS          Mode: GPIO_Mode_Out_PP
     */
...
void SPIx_DMAy_Read(uint32 srcAddr, uint32 dstAddr, uint16 count)
{
    ...
    dma_tx_init_struct.memory_addr = (uint32)(&dstAddr);
    ...
    dma_flag_clear(DMA0, DMA_CH2, DMA_FLAG_G); // 清RX全局中断标志
    ...
    while (!(spiCB.dmaFinished && spiCB.tx_dmaFinished))
        ;
    ...
    {
// DMA搬运做超时处理，超过门限则再次进行数据搬运，防止死机
// "#define _DMA_MOVE_BYTES_COUNT_TOP_          0xE000"超时值跟此宏密切相关，与主频也有关联，不可随意修改
#warning "此处计算需要注意主频与_DMA_MOVE_BYTES_COUNT_TOP_大小"
        if (number++ > 280000)
        {
            number = 0;
...
uint8 SPI_FlashReadForCrc8(uint32 datalen)
{
    ...
    while (len--)
    {
        //      dataTemp = SPI_FLASH_ReadWord(SPI_FLASH_APP_DATA_ADDEESS + addrIndex);
        addrIndex += 4;

        // 大端模式
        buf[0] = (uint8)(dataTemp >> 24);
        buf[1] = (uint8)(dataTemp >> 16);
        buf[2] = (uint8)(dataTemp >> 8);
        buf[3] = (uint8)dataTemp;
