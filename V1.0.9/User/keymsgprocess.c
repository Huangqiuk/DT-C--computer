...
//	1000,   	// KEY_NAME_DEC
//	1000,   	// KEY_NAME_INFO
//	1000,   	// KEY_NAME_ADD_DEC
//	1000,		// KEY_NAME_ADD_INFO
};
#endif
...
//	100,		// KEY_NAME_DEC
//	300,		// KEY_NAME_INFO
//	300,		// KEY_NAME_ADD_DEC
//	300,		// KEY_NAME_ADD_INFO
};
#endif
/******************************************************************************
* 【内部接口声明】
...
void KEYMSG_Init(void)
{
	...
	// 启动按键扫描 启动按键扫描定时器任务，定时扫描按键状态并执行按键处理回调函数
	TIMER_AddTask(TIMER_ID_KEY_SCAN,   //按键扫描任务
					KEY_SCAN_TIME,             //20ms 
					KEYMSG_CALLBACK_Scan,      //按键处理回调函数 	keyMsgCB.scanRequest = TRUE;     
					0,                         //回调函数参数
					TIMER_LOOP_FOREVER,        //无限循环
					ACTION_MODE_DO_AT_ONCE);   //立即执行
}
...
{
  //这段代码是将接受相同参数的函数指针 fun 赋值给了结构体 keyMsgCB 中的函数指针 KeyUpdate。这样，函数指针 KeyUpdate 就指向了传入的函数 fun。
...
void KEYMSG_ScanProcess(void)
{
	...
	//在KEY_Init()中已经注册了KEY_CALLBACK_Scan函数，在这里调用，将里面的*p8bitKeyValueArray = keyValue;等于keyMsgCB.keyValue= keyValue获取键值
	...
	for(i=0; i<KEY_NAME_MAX; i++)
	{
		...
		// 获取当前操作中的按键状态计数值指针stateCount[KEY_INFO_SIZE];
		...
		if(0 == currentState)
		{
			...
			else if(currentCount > 1)
			{
			...
			#endif
				{
					...
					// 当 + 键或 - 键长按时，屏蔽 UP 消息
//					if((KEY_NAME_ADD == i)||(KEY_NAME_DEC == i))
//					{
//						KEYMSG_SetMaskRequest((KEY_NAME_E)i, KEY_MSG_UP);
//					}
				#endif
				#if 0==KEY_MSG_THROW_KEY_UP_AFTER_REPEAT	// KEY_UP 屏蔽位置位时，不抛出 KEY_UP 消息
...
//{
//	keyMsgCB.addInfoKeyUpRercoveryRequest = state;
//}

//// 获取+ i键弹起恢复请求
//BOOL KEYMSG_GetAddInfoKeyUpRecoveryRequest(void)
//{
//	return keyMsgCB.addInfoKeyUpRercoveryRequest;
//}
