...
#define KEYMSGDEBUG(fsm,...)	//printf(fsm,##__VA_ARGS__);
...
void KEYMSG_Init(void)
{
	...
	//启动按键扫描
	TIMER_AddTask(TIMER_ID_KEY_SCAN, 20, KEYMSG_CALLBACK_Scan, true, TIMER_LOOP_FOREVER, ACTION_MODE_ADD_TO_QUEUE);
...
void KEYMSG_ScanProcess(void)
{
	...
	(*keyMsgCB.KeyUpdate)(keyMsgCB.keyValue);//滤波后的键值
	//KEYMSGDEBUG("Key%x\r\n", keyMsgCB.keyValue[0]);
	//Show_Bin(keyMsgCB.keyValue[0]);
	return;
...
uint8_t KEYMSG_ThrowMsg(KEY_NAME_E keyName, KEY_MSG_E keyMsg)
{
	...
	// 添加到队列中
	//---------------在注册长按消息的时候，抛出长按消息后会屏蔽一次UP消息-------------------
	//24/3/9 - 高富轲
	if(KEY_MSG_HOLD == keyMsg && KEYMSG_ServiceDoNothing != keyMsgCB.keyMsgInfo[keyName].service[KEY_MSG_HOLD]){//当前消息是长按，且注册了长按消息
			//屏蔽KEY_UP消息
			//KEYMSG_SetMaskRequest((KEY_NAME_E)keyName, KEY_MSG_UP);		
	}
